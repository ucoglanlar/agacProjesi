  	...
  	if(!alreadyChild){
  		...
  		if(count < internalSize){
  			...
  			for(int i = count-2; i >= 0; i--){
  				...
  				child->setRightSibling(children[i + 1]);		  				
...
bool InternalNode::insertLeftSibling(BTreeNode* value){
	...
	if(leftCount == internalSize){
		...
		//temp array
		int* tempKeys = new int[count + 1];
		BTreeNode** tempChildren = new BTreeNode*[count + 1];

		//copy over values in leftInternal to temp[]
		for(int i = 0; i < count; i++){
		...
		insertSortedArray(tempChildren, tempKeys, value, value->getMinimum(), count);
		...
		leftInternal->insert(tempChildren[0]);
...
bool InternalNode::insertRightSibling(BTreeNode* value){
